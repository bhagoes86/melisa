<?php foreach ($content as $row): ?>
    <?php if ($row->type == 0) { ?>
        ...
        <div class="image link-image" style="border-bottom: 0px;">
            <a href="javascript:void(0)" id="pic-bookmark-activate-in-bookmark" data-id="<?php echo $row->id_content ?>">
            ...
            <div class="description">
                <a href="javascript:void(0)" id="btn-bookmark-activate-in-bookmark" data-id="<?php echo $row->id_content ?>"><?php echo word_wrap(nl2br(auto_link($row->title)), 40); ?></a>
            </div>
    ...
    <?php } elseif ($row->type == 5) { ?>
        ...
        <div class="text" style="border: 1px solid #bbb;color: #bbb;">
            <a id="btn-bookmark-activate-in-bookmark" data-id="<?php echo $row->id_content ?>"><i class="icon-file-pdf"></i>&nbsp;<?php echo $row->file ?></a>
        </div>
    <?php } elseif ($row->type == 5) { ?>
        <!--slideshare-->
        <div class="text" id="wall-bookmark-viewer-in-bookmark-<?php echo $row->id_content ?>" data-id="<?php echo $row->id_content ?>" style="display: none;padding: 0px;vertical-align: middle;margin: 0px;background: rgba(0,0,0,0.10);"></div>                            
...
<script>
    ...
    $('a#btn-bookmark-activate-in-bookmark').click(function(){
        var id_content = $(this).attr('data-id');
        $('#wall-bookmark-viewer-in-bookmark-'+id_content).slideToggle('fast');
        $('#wall-bookmark-viewer-in-bookmark-'+id_content).load("<?php echo site_url('forum/wall_content_player') ?>/"+id_content);           
        return false;
    });
    //picture activate preview
    $('a#pic-bookmark-activate-in-bookmark').click(function(){
        var id_content = $(this).attr('data-id');
        $('#wall-bookmark-viewer-in-bookmark-'+id_content).slideToggle('fast');
        $('#wall-bookmark-viewer-in-bookmark-'+id_content).load("<?php echo site_url('forum/wall_content_player') ?>/"+id_content);
        return false;
