        ...
        <?php foreach ($list_assignment_result as $result): ?>
            ...
            <tr>
                ...
                <td style="width: 30px;border: 1px solid white;vertical-align: middle;background-color:rgba(0, 0, 0, 0.0666667);">
                    ...
                    <a title="hapus grup" href="javascript:void(0)" id="btn-delete" data-id="<?php echo $result->id_assignment_student; ?>"><i class="icon-remove fg-color-red"></i></a>
               </td>
            </tr>
           <?php } ?>
        <?php endforeach; ?>
...
<script type="text/javascript">
    ...
    $('#accept-confirm-message').click(function(){
            ...
            $.ajax({
                type:'POST',
                url:"<?php echo site_url('assignment/delete_result') ?>/"+id_result,
                data:id_result,
                success:function (data) {
                    $('#content-right').load("<?php echo site_url('assignment/list_result') ?>/"+<?php echo $assignment_id; ?>,function(){
    ...
    $('a#btn-edit').click(function(){
        ...
        var id_result = $(this).attr('data-id');
        $('#content-right').load("<?php echo site_url('assignment/show_form_edit_result') ?>/"+id_result,function(){
            $('#loading-template').fadeOut("slow");
        });
    });
    ...
    $('#quiz-add-form').click(function(){
        ...
        $('#loading-template').show();
        $('#content-right').load("<?php echo site_url('assignment/show_form_add_result')."/".$assignment_id ?>",function(){
            $('#loading-template').fadeOut("slow");
        });
    });
