...
<table class="striped" id="my-table">
    <thead>
        <tr>
            <th><b>Daftar Fakultas</b></th>
            <th class="right"><b>Singkatan</b></th>
        ...
        <?php foreach ($select as $row): ?>
            <tr>
                ...
                <td style="width: 30px;border: 1px solid white;vertical-align: middle;background-color:rgba(0, 0, 0, 0.0666667);">
                    <a title="Edit" href="javascript:void(0)" id="btn-edit" data-kuliah="<?php echo $row->jmlh_kuliah; ?>" data-konten="<?php echo $row->jmlh_konten; ?>" data-id="<?php echo $row->id_faculty; ?>"><i class="icon-pencil fg-color-pink"></i></a>
                    <a title="Hapus" href="javascript:void(0)" id="btn-delete" data-kuliah="<?php echo $row->jmlh_kuliah; ?>" data-konten="<?php echo $row->jmlh_konten; ?>" data-id="<?php echo $row->id_faculty; ?>"><i class="icon-remove fg-color-red"></i></a>  
                </td>
            </tr>
        <?php endforeach; ?>
    </tbody>
...
<script type="text/javascript">
    $('#faculty-add-form').click(function(){
        $('#message').html("Loading Data");
        $('#loading-template').show();
        $('#content-right').load("<?php echo site_url('course/form_add_faculty') ?>",function(){
    ...
    $('a#btn-edit').click(function(){
        ...
        $('#content-right').load("<?php echo site_url('course/form_edit_faculty') ?>/"+id,function(){
            $('#loading-template').fadeOut("slow");
        });
    });
    $('a#btn-delete').click(function(){
        if($(this).attr('data-konten')>0 || $(this).attr('data-kuliah') >0){
            $('#message-confirm1').html('Fakultas ini tidak bisa di hapus.');
    ...
    $('#accept-confirm-message').click(function(){
        ...
        $.ajax({
            type:'POST',
            url:"<?php echo site_url('course/delete_faculty') ?>/"+id,
            data:id,
