...
class Model_quiz extends CI_Model {
    ...
    function select_soal_by_quiz_and_id($id_quiz, $id_soal, $type_request){
        $this->db->select('*');
        $this->db->from('quiz_soal');
        $this->db->where('id_soal', $id_soal);
        $this->db->where('quiz_id', $id_quiz);
    ...
    function delete_quiz_summary_from_soal($id_quiz_resource, $data) {
        $this->db->where('summary_id', $id_quiz_resource);
        $this->db->update('quiz_soal', $data);
    }
