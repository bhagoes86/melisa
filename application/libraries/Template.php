...
class Template
{
	...
	{
		$identifier = strpos($files, '::');
		$file = substr($files, $identifier+2);
		if($identifier){
			$location = substr($files, 0, $identifier);
			if($location != 'theme') // kalo bukan theme berarti nama modul
				$files = base_url().MODULE_PATH.$location.'/css/'.$file;
		}

		self::set_metadata('stylesheet', $files, 'css');
	...
	public function append_js($files, $min_file = null, $group = 'extra')
	{
		...
		if($identifier){
			$location = substr($files, 0, $identifier);
			if($location != 'theme') // kalo bukan di dalam theme berarti di dalam modul
				$files = base_url().MODULE_PATH.$location.'/js/'.$file;
		}

		self::set_metadata('javascript', $files, 'javascript');
	...
	public function set_metadata($name, $content, $type = 'meta')
	{
		...
		switch($type)
		{
			...
			case 'javascript':
			$this->_metadata['header'][$content] = '<script type="text/'.$type.'" src="'.$content.'"></script>';
			break;

			case 'css':
			$this->_metadata['header'][$content] = '<link rel="'.$name.'" href="'.$content.'" type="text/'.$type.'" />';
			break;			
		}
